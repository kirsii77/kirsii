...
from PyQt5.QtWidgets import (QGroupBox, QApplication, QWidget, 
                        QPushButton, QHBoxLayout, QVBoxLayout, QLabel, QRadioButton,
                        QButtonGroup)
...
q1 = Question('Какого цвета нет на флаге Гермаинии', 'белый','чёрный', 'красный', 'жёлтый')

q2 = Question('Ответь "да"', 'да', 'не хочу', 'нет', 'не буду')





questions_spisok = list()
questions_spisok.append(q)
questions_spisok.append(q1)
questions_spisok.append(q2)
shuffle(questions_spisok)

def next_question():
    cur_question = randint(0, len(questions_spisok) - 1)
    ask(questions_spisok[cur_question])
    window.total += 1
